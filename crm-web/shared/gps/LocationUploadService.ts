...
import { UPLOAD_RETRY } from './config';

/**
 * Handles uploading location batches to the backend location-tracking service.
 *
 * Shared between web and mobile — uses the standard Fetch API which is
 * available in both React Native and modern browsers.
 *
 * Features:
...
export class LocationUploadService {
  ...
  /**
   * Fire-and-forget upload using navigator.sendBeacon().
   * Used as a last-ditch attempt to flush data when the browser tab is closing.
   * Returns true if the browser accepted the beacon for delivery.
   */
  sendBeacon(batch: LocationBatch, authToken: string): boolean {
    ...
    return navigator.sendBeacon(
      `${this.apiBaseUrl}/api/v1/location-tracking/batch`,
      blob,
    );
  }
