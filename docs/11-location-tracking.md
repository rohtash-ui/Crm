...
mobile device to the backend, through the event backbone, and into downstream
consumers.

## Design goals

1. **Always-on** — GPS is fetched continuously while the app is in foreground
   or background, within configurable active hours.
2. **Zero data loss** — Updates are buffered locally on-device (SQLite) and
   uploaded in batches with retries. No GPS fix is ever silently dropped.
...
│                                                           │
│  ┌─────────────────────┐   ┌──────────────────────────┐   │
│  │  GpsTrackingService │──▶│  LocationBufferService   │   │
│  │  (watchPosition /   │   │  (SQLite offline queue)  │   │
│  │   Background Geo)   │   └────────────┬─────────────┘   │
│  └─────────────────────┘                │                 │
│                                         ▼                 │
│                              ┌──────────────────────┐     │
...
| `GET` | `/api/v1/location-tracking/config` | Fetch tenant GPS tracking config (used by mobile on startup). |
...
- **`tracking_config`** — Per-tenant configuration. The mobile client fetches
  this on startup and merges it with local defaults.
