...
export async function GET(request: Request) {
  ...
  const [existingBookings, calConn] = await Promise.all([
  ...
  // FreeBusy: use if Calendar is configured and the host has connected it
  let busyPeriods: { start: string; end: string }[] = [];
  if (isGoogleCalendarConfigured() && calConn) {
    busyPeriods = await getFreeBusyTimes(user.id, dayStart, dayEnd);
  }
...
export async function POST(request: Request) {
  ...
  const {
    username, slug, startTime,
    guestName, guestEmail, guestPhone, guestTimezone, notes,
  } = body;
  ...
  // Verify host is available on this day
  const avail = await prisma.availability.findFirst({
    where: { userId: user.id, dayOfWeek: start.getDay(), isActive: true },
  });
  if (!avail) {
  ...
  // Free events — confirm immediately (no calendar event yet; created on confirmation)
  if (eventType.price === 0) {
    await prisma.booking.update({ where: { id: booking.id }, data: { status: "CONFIRMED" } });

    // Create calendar event immediately for free bookings
    if (isGoogleCalendarConfigured()) {
      ...
      if (calConn) {
        try {
          ...
          const calEvent = await createCalendarEvent(user.id, {
            summary:     `${eventType.title} with ${guestName.trim()}`,
          ...
          console.error("[Booking/Free] Calendar event creation failed:", err);
        }
      }
    }
  ...
  // Paid events — create pending payment record
